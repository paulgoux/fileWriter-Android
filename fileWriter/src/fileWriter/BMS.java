...
public class BMS extends MouseFunctions{
	...
	dockUpdateW,dockUpdateA,showNetworks,border,updateKey,keyPressed,getKey,key1,kp,mup;
	...
	public void setupDock(){
		dock = new Dock(0,applet.height -42,applet.width,24,this);
	...
	public void closeMouseFunctions() {
		if(debug) {
			if(click)applet.println("bms run menu click");
			if(mousePressed)applet.println("bms run menu mdown");
			if(applet.mousePressed)applet.println("bms run menu appmdown");
			if(mouseReleased)applet.println("bms run menu mup");
			if(mouseDragged)applet.println("bms run menu drag");
		}
		click = false;
		mouseReleased = false;
	};
...
//		click = false;
	...
	public void runMenus() {
		...
		dock.drawItems();
		if(debug)PApplet.println("bms display 16");
